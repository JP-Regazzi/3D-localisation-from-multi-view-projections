...
def parse_annotation(label_dir, cls_to_ind):
    ...
    for label_file in os.listdir(label_dir):
        ...
        for line in open(label_dir + "/" + label_file).readlines():
            ...
            if int(cls) in cls_to_ind.values():      # if cls in cls_to_ind.keys():
                theta_loc = -float(line[1]) + 3*np.pi / 2.
                ...
                obj = {'name': cls,
                       ...
                       'dims': np.array([float(number) for number in line[6:9]]),
                       'theta_loc': theta_loc
                       }
...
def get_obj_patch(image_dir, obj, target_size = (224, 224)):
    ...
    img = copy.deepcopy(img[min(ymin, ymax):max(ymax, ymin) + 1, min(xmin, xmax):max(xmax, xmin) + 1]).astype(np.float32)
...
def get_dect2D_data(box2d_file,classes):
    ...
    for line in open(box2d_file):
        ...
        if cls in classes:
            box_2D = np.asarray(line[2:6],dtype=np.float)
